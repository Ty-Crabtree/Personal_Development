...
def version_calculator():
    ...
    c_count = os.popen(f'git rev-list --count  {current_branch}').read().strip()
    logging.info(f'Total commits to current branch: {c_count}')
    d_count = os.popen(f'git rev-list --count  develop').read().strip()
    logging.info(f'Total commits to develop branch: {d_count}')
    m_count = os.popen(f'git rev-list --count  master').read().strip()
